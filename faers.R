...
#
# Source data:  2019Q3 data files (REAC19Q3.txt - 452K entries / 56MB, DEMO19Q3.txt - 1.5M entries / 63MB)
#
...
ev.freq.heat <- ev.freq[ev.heatmap$rowInd,ev.heatmap$colInd]
...
#ev.top.hist <- hist(ev.top.vec[ ev.top.vec != 0], main="Drug Event Frequency Histogram", xlab="Event frequency for each country", ylab="Counts", col="skyblue")  # exclude zero counts
...
ev.distinct <- ev.freq.heat[apply(ev.freq.heat, 1, function(x){ any(x > cutfreq); }),]
#ev.top.distinct <- ev.top[apply(ev.top, 1, function(x){ any(x > cutfreq); }),]
